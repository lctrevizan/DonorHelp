...
const InputField: React.FC<InputFieldProps> = ({ value, onChangeText, placeholder, style }) => {
  ...
  const animatedIsFocused = useMemo(() => new Animated.Value(value ? 1 : 0), [value]);
...
const CardItem: React.FC<CardItemProps> = React.memo(({ card, updateCard, deleteCard }) => {
...
export default function TabTwoScreen() {
  ...
  const addCard = useCallback(() => {
    const newCard: Card = { id: Date.now(), date: '', hemoglobin: '', iron: '', bloodPressure: '' };
